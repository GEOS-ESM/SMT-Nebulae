...
# ===============================
runs = {
    "Fortran": xr.open_dataset(f"{CPU_PATH}/{exp}_"+param+"_multi_fortran_cpu_"+levs+".nc4"),
    "CPU":     xr.open_dataset(f"{CPU_PATH}/{exp}_"+param+"_multi_dsl_cpu_"+levs+".nc4"),
    "GPU":     xr.open_dataset(f"{GPU_PATH}/{exp}_"+param+"_multi_dsl_gpu_"+levs+".nc4"),
}

surf_runs = {
    "Fortran": xr.open_dataset(f"{CPU_PATH}/{exp}_"+param+"_surf_fortran_cpu_"+levs+".nc4"),
    "CPU":     xr.open_dataset(f"{CPU_PATH}/{exp}_"+param+"_surf_dsl_cpu_"+levs+".nc4"),
    "GPU":     xr.open_dataset(f"{GPU_PATH}/{exp}_"+param+"_surf_dsl_gpu_"+levs+".nc4"),
}
...
for name, ds in runs.items():
    ...
    for var in variables:
        profiles[name][var] = time_mean(ds, var, t_start, t_end)
...
colors = {"Fortran": "k", "CPU": "red", "GPU": "blue"}
...
if plt_profiles and exp == "bomex":
    ...
    ax = axs[0]
    for name in runs:
        profile(ax, derived[name]["theta_l"][::-1], z, name, colors[name], marker="o")
    ...
    ax = axs[1]
    for name in runs:
        profile(ax, derived[name]["QT"][::-1], z, name, colors[name], marker="o")
    ...
    ax = axs[2]
    for name in runs:
        profile(ax, profiles[name]["QL"][:,0,0][::-1], z, name, colors[name], marker="o")
    ...
    ax = axs[3]
    for name in runs:
        profile(ax, profiles[name]["CLOUD"][:,0,0][::-1], z, name, colors[name], marker="o")
    ...
    ax = axs[4]
    for name in runs:
        profile(ax, derived[name]["LWC"][::-1], z, name, colors[name], marker="o")
    ...
    ax = axs[5]
    for name in runs:
        profile(ax, profiles[name]["MU"][:,0,0][::-1], z, name, colors[name], marker="o")
    ax.set_title("f)", loc="left", fontweight="bold")
    ax.set_title("Updraft Mass Flux")
    ax.legend(); ax.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig(f"/Users/kfandric/SMT-Nebulae/plotting_tools/scm_{exp}_profiles_gpu_"+levs+".png", dpi=300)
    plt.show()
...
if hovmoller and exp != "armtwp_ice":
    ...
    nvar = len(variables)
    fig, axs = plt.subplots(nvar, 5, figsize=(20, 4*nvar), sharex=True, sharey=True)
    ...
    for i, var in enumerate(variables):
        ...
        datasets = [f90_data, cpu_data, gpu_data, diff_cpu, diff_gpu]
        titles = ["Fortran", "CPU", "GPU", "CPU-F90", "GPU-F90"]
        cmaps = [None, None, None, "RdBu_r", "RdBu_r"]
    ...
    plt.tight_layout()
    plt.savefig(f"/Users/kfandric/SMT-Nebulae/plotting_tools/scm_{exp}_hovmoller_gpu_"+levs+".png", dpi=300)
    plt.show()
